module.exports = (async function() {
  ...
  const kpi = "exportacionesenergia"
  const url = 'https://infra.datos.gob.ar/catalog/sspm/dataset/75/distribution/75.3/download/exportaciones-mensual.csv'
  const payload = await parsers.datosGobarCSV(0,48,url)
  const post = {
    kpi,
    t: "Exportaciones Energía",
    st: "Gas y Petroleo",
    sd: "",
    ...
    dimensions: [
      ...
      },
      {
      ...
      },      
       {
        ...
        data: await parsers.datosGobarCSV(0,49,url),
        color: "rgba(46,120,210,0.3)",
      },
      {
        label: "Gas y Otros",
        data: await parsers.datosGobarCSV(0,50,url),
        color: "rgba(46,120,210,0.3)",
      },
      {
        label: "Combustibles",
