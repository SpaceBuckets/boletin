  ...
  <div>
    <section class="emaerto">
                 <div class="title-container"> 
                   <h2><span style="text-transform:uppercase">{{currActive}}</span>. {{econ[currActive]}} </h2>
...
export default {
  data() {
    return {
      currActive: "emae",
      econdates: {
        emae: emaeDates[emaeDates.length - 1]
          .replace(/-/g, "/")
          .split("/")
          .reverse()
          .join("/"),
        ipi: ipiDates[ipiDates.length - 1]
          .replace(/-/g, "/")
          .split("/")
          .reverse()
          .join("/"),
        ucii: uciiDates[uciiDates.length - 1]
          .replace(/-/g, "/")
          .split("/")
          .reverse()
          .join("/"),
        isac: isacDates[isacDates.length - 1]
          .replace(/-/g, "/")
          .split("/")
          .reverse()
          .join("/"),
