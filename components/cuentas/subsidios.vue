<template>
  <div style="position: relative" class="">
    <div class="title-container" style="min-width: 100%">
      <h2><span>Subsidios Económicos</span>. Energía y Transporte</h2>
    </div>
    <div class="chartcont">
       ...
       <p>El EMAE es un <strong>indicador provisorio de la evolución del PBI</strong> que ofrece una pauta de la actividad económica real. Se elabora con información parcial calculando la <strong>suma del valor agregado de las actividades económicas</strong>.</p> 
        ...
        <div class="flexedcontent">
          ...
          >
            ...
            <div>
              {{ chartData.datasets[1].data.filter((val, index, arr) => index > arr.length - 8).reverse()[i].toFixed(2) }}
            </div>
...
export default {
  ...
  methods: {
    ...
    getVariation(i) {
      var currentNum = this.chartData.datasets[1].data.filter((val, index, arr) => index > arr.length - 8).reverse()
      var A = currentNum[i] 
